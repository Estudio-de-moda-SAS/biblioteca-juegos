...
interface CardProps {
  ...
  isError: boolean; isDisabled: boolean; primaryColor: string; cardBackImage: string; onClick: () => void
}

const MemoryCard: FC<CardProps> = ({ card, isFlipped, isMatched, isError, isDisabled, primaryColor, cardBackImage, onClick }) => {
      ...
      <div className={`mm-card-inner${faceUp ? ' mm-up' : ''}`}>
        ...
        <div className="mm-face" style={{
          ...
          <img
            src={cardBackImage}
            alt="Reverso"
            style={{ width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none', opacity: isError ? 0.4 : 1, transition: 'opacity 0.2s' }}
            draggable={false}
          />
          {isError && (
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(239,68,68,0.35)' }} />
          )}
        </div>
        {/* Faceup */}
...
function MemoryMatch({
  ...
  logo,
  backgroundImage,
  cardBackImage            = 'https://picsum.photos/seed/card-back/200/200',
  cards: cardsProp,
  ...
  function handleCardClick(uid: string, pairId: string) {
    if (lockedRef.current || matchedRef.current.has(pairId) || flippedRef.current.indexOf(uid) !== -1 || flippedRef.current.length >= 2) return
    const newFlipped = [...flippedRef.current, uid]
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100vh', background:`linear-gradient(180deg, ${secondaryColor} 0%, #0f0f1a 100%)` }}>
      {/* Header */}
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:24, overflowY:'auto' }}>
        <div style={{
          display:'grid', gridTemplateColumns:`repeat(${cols}, 1fr)`, gap:12, width:'100%',
          maxWidth: bp === 'mobile' ? 420 : bp === 'tablet' ? 620 : 560,
        }}>
          {cards.map(card => (
            <MemoryCard
              key={card.uid} card={card}
              isFlipped={flippedUids.indexOf(card.uid) !== -1}
              isMatched={matchedPairIds.has(card.pairId)}
              isError={errorUids.indexOf(card.uid) !== -1}
              isDisabled={isLocked && flippedUids.indexOf(card.uid) === -1}
...
;(MemoryMatch as any).schema = {
  ...
  properties: {
    ...
    cardBackImage:   { title: 'Imagen reverso de cartas (URL)', type: 'string', widget: { 'ui:widget': 'image-uploader' } },
